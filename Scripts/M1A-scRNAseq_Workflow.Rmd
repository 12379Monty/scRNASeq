...
```{r setParam, echo=F}
 ...
 TABLE_DIR <- file.path(WRKDIR, 'Scripts', 'tables', 'M1A')
...
```{r downloadData, eval=F, echo=T, cache=T, cache.vars=''}
...
```{r loadCounts, cache=F, cache.vars='',eval=F}
...
```{r libplothsc, echo=F, include=F, eval=F, fig.height=6, fig.width=12}
 load(file.path(WRKDIR, 'Data', 'GSE61533.sce'))

 par(mfrow=c(1,2),oma=c(0,0,2,0))
 hist(GSE61533.sce$total_counts/1e6, xlab="Library sizes (millions)", main="", 
    breaks=20, col="grey80", ylab="Number of cells")
 hist(GSE61533.sce$total_features, xlab="Number of expressed genes", main="", 
    breaks=20, col="grey80", ylab="Number of cells")
 mtext(side=3, outer=T, 
  "Fig. 1: Library sizes (left) and number of expressed genes (right) for all cells in the HSC dataset.")
...
```{r libplothsc2, echo=T, fig.height=6, fig.width=12}
...
```{r controlplothsc, echo=F, eval=F, fig.width=12, fig.height=6}
 load(file.path(WRKDIR, 'Data', 'GSE61533.sce'))

 #libsize.drop <- isOutlier(GSE61533.sce$total_counts, nmads=3, type="lower", log=TRUE)
 #feature.drop <- isOutlier(GSE61533.sce$total_features, nmads=3, type="lower", log=TRUE)
 
  par(mfrow=c(1,2),oma=c(0,0,2,0))
 hist(GSE61533.sce$pct_counts_feature_controls_Mt, xlab="Mitochondrial proportion (%)", 
     ylab="Number of cells", breaks=20, main="", col="grey80")
 hist(GSE61533.sce$pct_counts_feature_controls_ERCC, xlab="ERCC proportion (%)", 
     ylab="Number of cells", breaks=20, main="", col="grey80")
 mtext(side=3, outer=T,
 "Fig. 2: Proportion of reads mapped to mitochondrial genes (left) or spike-in transcripts (right) across all cells in the HSC dataset.")
...
```{r controlplothsc2, echo=T, fig.height=6, fig.width=12}
 load(file.path(WRKDIR, 'Data', 'GSE61533.Counts.frm'))
...
```{r outlierDrop, echo=F, eval=F}
 ...
 mito.drop <- isOutlier(GSE61533.sce$pct_counts_feature_controls_Mt, nmads=3, type="higher")
 colnames(GSE61533.sce)[mito.drop]
 spike.drop <- isOutlier(GSE61533.sce$pct_counts_feature_controls_ERCC, nmads=3, type="higher")
...
nohup sudo Rscript -e "knitr::knit2html('M1A-scRNAseq_Workflow.Rmd')" > M1A-scRNAseq_Workflow.log  &
