...
```{r Prelims, include=F, echo=F, results='hide', message=F,warning=F}
  ...
  cache_DIR <- file.path(WRKDIR, 'Scripts', 'cache/M4B/')
  suppressMessages(dir.create(cache_DIR, recursive=T))
  opts_chunk$set(cache.path=cache_DIR)

  figure_DIR <- file.path(WRKDIR, 'Scripts', 'figure/M4B/')
...
```{r setParam, echo=F}
 ...
 #TABLE_DIR <- file.path(WRKDIR, 'Scripts', 'tables', 'M4B')
...
```
 ...
 nohup sudo Rscript -e "knitr::knit2html('M4B-sconeTutorial.Rmd')" > M4B-sconeTutorial.log  &
