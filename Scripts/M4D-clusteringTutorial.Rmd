...
```{r Prelims, include=F, echo=F, results='hide', message=F,warning=F}
  ...
  cache_DIR <- file.path(WRKDIR, 'Scripts', 'cache/M4D/')
  suppressMessages(dir.create(cache_DIR, recursive=T))
  opts_chunk$set(cache.path=cache_DIR)

  figure_DIR <- file.path(WRKDIR, 'Scripts', 'figure/M4D/')
...
```{r setParam, echo=F}
 ...
 #TABLE_DIR <- file.path(WRKDIR, 'Scripts', 'tables', 'M4D')
...
```{r GlobalOptions, results="hide", include=FALSE, cache=FALSE}
 SKIP.THIS <- function() {
    ...
    cache.path = "clusterExperimentTutorial_cache/", 
    fig.path="clusterExperimentTutorial_figure/",error=FALSE, #make it stop on error
...
```{r mergeClustersPlot, fig.height=12, fig.width=12}
...
```{r mergeClusters, fig.height=12, fig.width=12}
...
```{r mergeClusters_plot,fig.width=12, fig.height=12}
...
```{r mergeClusters_ex,fig.width=12, fig.height=12}
...
```{r mergeClusters_redo,fig.width=12, fig.height=12}
...
```
 ...
 nohup sudo Rscript -e "knitr::knit2html('M4D-clusteringTutorial.Rmd')" > M4D-clusteringTutorial.log  &
