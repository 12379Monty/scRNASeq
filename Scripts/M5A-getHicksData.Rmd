### Hicks scBatchPaper: Get Data from Repositories 

## THIS DOESNT WORK.  ALL DATA ARE IN INACCESSIBLE DATA PACKAGES!

Reference:  [Hicks](https://github.com/stephaniehicks/scBatchPaper)
...
```{r Prelims, include=F, echo=FALSE, results='hide', message=FALSE}
  ...
  #figure_DIR <- file.path(WRKDIR, 'Scripts', 'figure/M5A/')
...
```{r loadLibs, echo=F}
 ...
 if(grepl('mnt100', WRKDIR)) suppressMessages(require(vcd))
...
As package cRNASeqMousDeng.esetMonoAllelic is unavailable, need to get Deng data else where.

#### Replace this:
```{r loadDeng, eval=F}
 # CANT FIND THIS DATASET
 suppressMessages(require(scRNASeqMousDeng.esetMonoAllelic))

 data("embryoMouseRPKM")

 # Expressions
 Deng.eset <- exprs(embryoMouseRPKM)
 Deng.log2E <- log2(Deng.eset+1)

 # Phenotype Data
 Deng.pd <- pData(embryoMouseRPKM)

 # calculate CDR
 Deng.pd$CDR <- colMeans(Deng.eset != 0)
 Deng.pd$CDRlt1 <- colMeans(Deng.eset > 1)
 Deng.pd$cS <- colSums(Deng.eset) # colSums on original scale

 Deng.pd$colMeans <- colMeans(Deng.log2E)

 tt <- levels(Deng.pd$time)
 Deng.pd$bin <- ifelse(Deng.pd$time %in% tt[1:2], "Bin A",
               ifelse(Deng.pd$time %in% tt[3:4], "Bin B",
               ifelse(Deng.pd$time %in% tt[5:7], "Bin C", "Bin D")))
 
 # calculate SVD
 dat <- sweep(Deng.eset[, Deng.pd$bin == "Bin A"], 1, 
              rowMeans(Deng.eset[, Deng.pd$bin == "Bin A"]), FUN = "-")
 sDeng.A <- svd(dat)

 dat <- sweep(Deng.eset[, Deng.pd$bin == "Bin B"], 1, 
              rowMeans(Deng.eset[, Deng.pd$bin == "Bin B"]), FUN = "-")
 sDeng.B <- svd(dat)

 dat <- sweep(Deng.eset[, Deng.pd$bin == "Bin C"], 1, 
        rowMeans(Deng.eset[, Deng.pd$bin == "Bin C"]), FUN = "-")
 sDeng.C <- svd(dat)

 dat <- sweep(Deng.eset[, Deng.pd$bin == "Bin D"], 1, 
        rowMeans(Deng.eset[, Deng.pd$bin == "Bin D"]), FUN = "-")
 sDeng.D <- svd(dat)
...
#### With this:
```{r intallDengLibs, eval=F}
 # DO ONCE
 library(devtools)
 install_github('kkdey/CountClust')
 install_github('kkdey/singleCellRNASeqMousDeng.eset2014') 
```
   
```{r loadDeng2, eval=T}
 # CANT FIND THIS DATASET
 suppressMessages(require(singleCellRNASeqMousDeng.eset2014))

 # Expressions
 Deng.eset <- exprs(Deng2014MouseESC)
 Deng.log2E <- log2(Deng.eset+1)

 # Phenotype Data
 Deng.pd <- pData(Deng2014MouseESC)
 ...
 # in this package Deng.pd has no tome!
 head(Deng.pd)

 table(Deng.pd$embryo_id)

 # Can we use embryo_id
REPLACE.THIS <- function() {
 tt <- levels(Deng.pd$time)
 Deng.pd$bin <- ifelse(Deng.pd$time %in% tt[1:2], "Bin A",
               ifelse(Deng.pd$time %in% tt[3:4], "Bin B",
               ifelse(Deng.pd$time %in% tt[5:7], "Bin C", "Bin D")))
} # WITH (MAY NEED TO COME BACK AND FILTER!!!!)
 tt <- levels(Deng.pd$embryo_id)
 
 Deng.pd$bin <- ifelse(Deng.pd$embryo_id %in% tt[1:3], "Bin A",
               ifelse(Deng.pd$embryo_id %in% tt[4:5], "Bin B",
               ifelse(Deng.pd$embryo_id %in% tt[6:7], "Bin C", "Bin D")))
 table(Deng.pd$bin)
...
#### Not available.
```{r loadJaitin, eval=F}
 suppressMessages(require(scRNASeqMouseJaitinSpleen))
...
# remove cells with a suppressMessage(require size of 0 and outlier cells
...
suppressMessage(require(scRNASeqMouseKumarPSC)
...
suppressMessage(require(SummarizedExperiment)
suppressMessage(require(patel2014gliohuman) # install_github("willtownes/patel2014gliohuman")
...
suppressMessage(require(scRNASeqHumanPatelGlioblastoma)
data("glioHumanTPM")
...
suppressMessage(require(scRNASeqMouseShalekDendritic)
...
suppressMessage(require(trapnell2014myoblasthuman) # install_github("stephaniehicks/trapnell2014myoblasthuman")
...
suppressMessage(require(scRNASeqMouseTreutleinLineage)
data("lungMouseFPKM")
...
suppressMessage(require(scRNASeqHumanBosePrinting)
...
suppressMessage(require(scRNASeqMouseBurnsInnerEar)
...
suppressMessage(require(scRNASeqHumanGuoGermCells)
...
suppressMessage(require(scRNASeqMouseKowalczykAging)
...
suppressMessage(require(scRNASeqHumanLengOscillatoryGenes)
...
```{r loadSatija}
suppressMessage(require(scRNASeqDaniSajitaSeurat))
...
suppressMessage(require(scRNASeqMouseZeiselCortex))
data("cortexMouseUMI")
...
### Set inital parameters
 ...
 ROOT <- '/SOMEWHERE/fcollin/SingleCell'

 EXTDATA_DIR <- file.path(ROOT,'Data')

 #TABLE_DIR <- file.path(WRKDIR, 'Scripts', 'tables', 'M5A')
...
nohup sudo Rscript -e "knitr::knit2html('M5A-getHicksData.Rmd')" > M5A-getHicksData.log  &
