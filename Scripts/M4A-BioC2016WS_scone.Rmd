...
```{r Prelims, include=F, echo=F, results='hide', message=F,warning=F}
  ...
  figure_DIR <- file.path(WRKDIR, 'Scripts', 'figure/M4A/')
  ...
  opts_chunk$set(error=F, message=F, warning=F)
...
```{r setParam, echo=F}
 ...
 TABLE_DIR <- file.path(WRKDIR, 'Scripts', 'tables', 'M4A')
...
```{r sconeOptions, results="hide", include=F, cache=F, results='hide', message=F}
 #knitr::opts_chunk$set(fig.align="center", cache=T,error=F,
                       #fig.width=6,fig.height=6,autodep=T,out.width="600px",
                       #out.height="600px", results="markup", echo=T, eval=TRUE)
 #options(getClass.msg=F) #???

 set.seed(6473) ## for reproducibility

 options(bitmapType='cairo')  ### fixes x11 disaplay problems!!!

 suppressMessages(require(scone))
 suppressMessages(require(RColorBrewer))
 suppressMessages(require(BiocStyle))
...
```{r fnr_vis, eval=T, fig.width= 8, fig.height= 4, out.width="800px",out.height="400px"}
...
```{r thresh, eval=T,fig.width= 6, fig.height= 4, out.width="600px",out.height="400px"}
...
nohup sudo Rscript -e "knitr::knit2html('M4A-BioC2016WS_scone.Rmd')" > M4A-BioC2016WS_scone.log  &
